...
class API(object):
    ...
    def __init__(self, key, secret, myproxy, srv = None,
        ...
        _setup_apiobj(self, self, [], myproxy)
...
def _setup_apiobj(self, api, path, myproxy):
    if self is not api:
        self._api = api
    ...
    for i in _APIS:
        ...
        if i[:lvl] == path and cur not in done:
            ...
            setattr(self, cur, _APIProxy(api, i[:lvl + 1], myproxy))
...
class _APIProxy(object):
    ...
    def __init__(self, api, path, myproxy):
        self.myproxy = myproxy
        _setup_apiobj(self, api, path, myproxy)
    ...
    def __call__(self, post = False, *args, **kargs):
        ...
        proxy = urllib2.ProxyHandler(self.myproxy)
